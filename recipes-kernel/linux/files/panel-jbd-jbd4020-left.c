...
static struct jbd4020_cfg_t jbd_panel_cfg[8] = {
    {8, 8, 0, 0, 8191, 60}, // R_panel
    {8, 8, 0, 1, 5707, 35}, // G_panel
    {8, 8, 0, 0, 5047, 30},	// B_panel
};
...
static int jbd4020_panel_probe(struct mipi_dsi_device *dsi)
{
	...
	jbd4020_panel_default_config(ctx);
	if (ret < 0) {
	...
	drm_panel_init(&ctx->panel, &dsi->dev, &jbd4020_funcs,
...
static int jbd4020_panel_remove(struct mipi_dsi_device *dsi)
{
	int ret = 0;
